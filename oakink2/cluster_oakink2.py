#!/usr/bin/env python3
"""STAGE 4 (embed + cluster) for OakInk2.

Embeds the unique cleaned strings with a sentence encoder, reduces with UMAP
...
artifacts. Exemplars carry full text: OakInk2 is CC BY-SA 4.0, which permits
redistribution with attribution and share-alike.

Reproducibility constants live at the top of the file. min_cluster_size is tuned
down from the HD-EPIC template because the OakInk2 corpus is smaller.

Outputs:
  oakink2_clusters.json   cluster id, label, sizes, exemplars, config
  oakink2_scatter.json    2D coords + cluster + count + text per point
  oakink2_stats.json      (patched) adds cluster summary
  .cache/oakink2_emb.npy  embeddings (gitignored)
"""
...
def main():
    ...
    print(f"unique strings to embed: {len(texts):,}")
    ...
    for cid in ids:
        ...
        order = idx[np.argsort(-sub_counts)]
